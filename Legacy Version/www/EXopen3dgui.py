...
class Settings:
    ...
    def __init__(self):
        self.mouse_model = visualization.gui.SceneWidget.Controls.ROTATE_CAMERA
        self.bg_color = visualization.gui.Color(1, 1, 1)
        ...
        self.sun_color = visualization.gui.Color(1, 1, 1)

        self.apply_material = True  # clear to False after processing
        self._materials = {
            Settings.LIT: visualization.rendering.MaterialRecord(),
            Settings.UNLIT: visualization.rendering.MaterialRecord(),
            Settings.NORMALS: visualization.rendering.MaterialRecord(),
            Settings.DEPTH: visualization.rendering.MaterialRecord()
        }
...
class AppWindow:
    ...
    def __init__(self, width, height):
        ...
        resource_path = visualization.gui.Application.instance.resource_path
        self.settings.new_ibl_name = resource_path + "/" + AppWindow.DEFAULT_IBL

        self.window = visualization.gui.Application.instance.create_window(
            "Open3D", width, height)
        w = self.window  # to make the code more concise

        # 3D widget
        self._scene = visualization.gui.SceneWidget()
        self._scene.scene = visualization.rendering.Open3DScene(w.renderer)
        ...
        # Widgets are laid out in layouts: visualization.gui.Horiz, visualization.gui.Vert,
        # visualization.gui.CollapsableVert, and visualization.gui.VGrid. By nesting the layouts we can
        ...
        self._settings_panel = visualization.gui.Vert(
            0, visualization.gui.Margins(0.25 * em, 0.25 * em, 0.25 * em, 0.25 * em))

        # Create a collapsible vertical widget, which takes up enough vertical
        # space for all its children when open, but only enough for text when
        # closed. This is useful for property pages, so the user can hide sets
        # of properties they rarely use.
        view_ctrls = visualization.gui.CollapsableVert("View controls", 0.25 * em,
                                         visualization.gui.Margins(em, 0, 0, 0))

        self._arcball_button = visualization.gui.Button("Arcball")
        self._arcball_button.horizontal_padding_em = 0.5
        self._arcball_button.vertical_padding_em = 0
        self._arcball_button.set_on_clicked(self._set_mouse_mode_rotate)
        self._fly_button = visualization.gui.Button("Fly")
        self._fly_button.horizontal_padding_em = 0.5
        self._fly_button.vertical_padding_em = 0
        self._fly_button.set_on_clicked(self._set_mouse_mode_fly)
        self._model_button = visualization.gui.Button("Model")
        ...
        self._sun_button = visualization.gui.Button("Sun")
        ...
        self._ibl_button = visualization.gui.Button("Environment")
        ...
        view_ctrls.add_child(visualization.gui.Label("Mouse controls"))
        ...
        h = visualization.gui.Horiz(0.25 * em)  # row 1
        h.add_stretch()
        ...
        h = visualization.gui.Horiz(0.25 * em)  # row 2
        h.add_stretch()
        ...
        self._show_skybox = visualization.gui.Checkbox("Show skymap")
        self._show_skybox.set_on_checked(self._on_show_skybox)
        view_ctrls.add_fixed(separation_height)
        view_ctrls.add_child(self._show_skybox)

        self._bg_color = visualization.gui.ColorEdit()
        self._bg_color.set_on_value_changed(self._on_bg_color)

        grid = visualization.gui.VGrid(2, 0.25 * em)
        grid.add_child(visualization.gui.Label("BG Color"))
        grid.add_child(self._bg_color)
        view_ctrls.add_child(grid)

        self._show_axes = visualization.gui.Checkbox("Show axes")
        self._show_axes.set_on_checked(self._on_show_axes)
        view_ctrls.add_fixed(separation_height)
        view_ctrls.add_child(self._show_axes)

        self._profiles = visualization.gui.Combobox()
        ...
        view_ctrls.add_fixed(separation_height)
        view_ctrls.add_child(visualization.gui.Label("Lighting profiles"))
        view_ctrls.add_child(self._profiles)
        self._settings_panel.add_fixed(separation_height)
        self._settings_panel.add_child(view_ctrls)

        advanced = visualization.gui.CollapsableVert("Advanced lighting", 0,
                                       visualization.gui.Margins(em, 0, 0, 0))
        advanced.set_is_open(False)

        self._use_ibl = visualization.gui.Checkbox("HDR map")
        self._use_ibl.set_on_checked(self._on_use_ibl)
        self._use_sun = visualization.gui.Checkbox("Sun")
        self._use_sun.set_on_checked(self._on_use_sun)
        advanced.add_child(visualization.gui.Label("Light sources"))
        h = visualization.gui.Horiz(em)
        h.add_child(self._use_ibl)
        h.add_child(self._use_sun)
        advanced.add_child(h)

        self._ibl_map = visualization.gui.Combobox()
        for ibl in glob.glob(visualization.gui.Application.instance.resource_path +
                             "/*_ibl.ktx"):
        ...
        self._ibl_intensity = visualization.gui.Slider(visualization.gui.Slider.INT)
        self._ibl_intensity.set_limits(0, 200000)
        self._ibl_intensity.set_on_value_changed(self._on_ibl_intensity)
        grid = visualization.gui.VGrid(2, 0.25 * em)
        grid.add_child(visualization.gui.Label("HDR map"))
        grid.add_child(self._ibl_map)
        grid.add_child(visualization.gui.Label("Intensity"))
        grid.add_child(self._ibl_intensity)
        advanced.add_fixed(separation_height)
        advanced.add_child(visualization.gui.Label("Environment"))
        advanced.add_child(grid)

        self._sun_intensity = visualization.gui.Slider(visualization.gui.Slider.INT)
        self._sun_intensity.set_limits(0, 200000)
        self._sun_intensity.set_on_value_changed(self._on_sun_intensity)
        self._sun_dir = visualization.gui.VectorEdit()
        self._sun_dir.set_on_value_changed(self._on_sun_dir)
        self._sun_color = visualization.gui.ColorEdit()
        self._sun_color.set_on_value_changed(self._on_sun_color)
        grid = visualization.gui.VGrid(2, 0.25 * em)
        grid.add_child(visualization.gui.Label("Intensity"))
        grid.add_child(self._sun_intensity)
        grid.add_child(visualization.gui.Label("Direction"))
        grid.add_child(self._sun_dir)
        grid.add_child(visualization.gui.Label("Color"))
        grid.add_child(self._sun_color)
        advanced.add_fixed(separation_height)
        advanced.add_child(visualization.gui.Label("Sun (Directional light)"))
        advanced.add_child(grid)

        self._settings_panel.add_fixed(separation_height)
        self._settings_panel.add_child(advanced)

        material_settings = visualization.gui.CollapsableVert("Material settings", 0,
                                                visualization.gui.Margins(em, 0, 0, 0))

        self._shader = visualization.gui.Combobox()
        ...
        self._material_prefab = visualization.gui.Combobox()
        ...
        self._material_color = visualization.gui.ColorEdit()
        self._material_color.set_on_value_changed(self._on_material_color)
        self._point_size = visualization.gui.Slider(visualization.gui.Slider.INT)
        self._point_size.set_limits(1, 10)
        self._point_size.set_on_value_changed(self._on_point_size)

        grid = visualization.gui.VGrid(2, 0.25 * em)
        grid.add_child(visualization.gui.Label("Type"))
        grid.add_child(self._shader)
        grid.add_child(visualization.gui.Label("Material"))
        grid.add_child(self._material_prefab)
        grid.add_child(visualization.gui.Label("Color"))
        grid.add_child(self._material_color)
        grid.add_child(visualization.gui.Label("Point size"))
        ...
        if visualization.gui.Application.instance.menubar is None:
            if isMacOS:
                app_menu = visualization.gui.Menu()
                app_menu.add_item("About", AppWindow.MENU_ABOUT)
                app_menu.add_separator()
                app_menu.add_item("Quit", AppWindow.MENU_QUIT)
            file_menu = visualization.gui.Menu()
            file_menu.add_item("Open...", AppWindow.MENU_OPEN)
            file_menu.add_item("Export Current Image...", AppWindow.MENU_EXPORT)
            if not isMacOS:
                file_menu.add_separator()
                file_menu.add_item("Quit", AppWindow.MENU_QUIT)
            settings_menu = visualization.gui.Menu()
            settings_menu.add_item("Lighting & Materials",
                                   AppWindow.MENU_SHOW_SETTINGS)
            settings_menu.set_checked(AppWindow.MENU_SHOW_SETTINGS, True)
            help_menu = visualization.gui.Menu()
            help_menu.add_item("About", AppWindow.MENU_ABOUT)

            menu = visualization.gui.Menu()
            if isMacOS:
            ...
            visualization.gui.Application.instance.menubar = menu
    ...
    def _apply_settings(self):
        ...
        c = visualization.gui.Color(self.settings.material.base_color[0],
    ...
    def _on_layout(self, layout_context):
        ...
        height = min(
            ...
            self._settings_panel.calc_preferred_size(
                layout_context, visualization.gui.Widget.Constraints()).height)
        self._settings_panel.frame = visualization.gui.Rect(r.get_right() - width, r.y, width,
                                              height)

    def _set_mouse_mode_rotate(self):
        self._scene.set_view_controls(visualization.gui.SceneWidget.Controls.ROTATE_CAMERA)

    def _set_mouse_mode_fly(self):
        self._scene.set_view_controls(visualization.gui.SceneWidget.Controls.FLY)

    def _set_mouse_mode_sun(self):
        self._scene.set_view_controls(visualization.gui.SceneWidget.Controls.ROTATE_SUN)

    def _set_mouse_mode_ibl(self):
        self._scene.set_view_controls(visualization.gui.SceneWidget.Controls.ROTATE_IBL)

    def _set_mouse_mode_model(self):
        self._scene.set_view_controls(visualization.gui.SceneWidget.Controls.ROTATE_MODEL)
    ...
    def _on_new_ibl(self, name, index):
        self.settings.new_ibl_name = visualization.gui.Application.instance.resource_path + "/" + name
        self._profiles.selected_text = Settings.CUSTOM_PROFILE_NAME
        self._apply_settings()
    ...
    def _on_menu_open(self):
        dlg = visualization.gui.FileDialog(visualization.gui.FileDialog.OPEN, "Choose file to load",
                             self.window.theme)
        dlg.add_filter(
    ...
    def _on_menu_export(self):
        dlg = visualization.gui.FileDialog(visualization.gui.FileDialog.SAVE, "Choose file to save",
                             self.window.theme)
    ...
    def _on_menu_toggle_settings_panel(self):
        ...
        visualization.gui.Application.instance.menubar.set_checked(
    ...
    def _on_menu_about(self):
        ...
        dlg = visualization.gui.Dialog("About")

        # Add the text
        dlg_layout = visualization.gui.Vert(em, visualization.gui.Margins(em, em, em, em))
        dlg_layout.add_child(visualization.gui.Label("Open3D GUI Example"))

        # Add the Ok button. We need to define a callback function to handle
        # the click.
        ok = visualization.gui.Button("OK")
...
def main():
    ...
    # for visualization.rendering and prepares the cross-platform window abstraction.
